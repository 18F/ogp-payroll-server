...
def counties(det):
    ...
    for county in det.xpath('location/state/countyIncludedList/countyName'):
        yield {'county': county.text,
               'state':
               county.iterancestors('state').__next__().find('stateName').text,
               'abbrev': abbrev, }
        ...
        for rate_type in ('occupation', 'rate', 'subrate'):
            for rate in raw_det.iter(rate_type + 'Rate'):
                ...
                for construction_type in construction_types(group):
                    result = {
                        'location_qualifier':
                        group.find('locationQualifier').text,
                        # TODO: Use loc qualifier to further filter the county list
                        'survey_location_qualifier':
                        raw_det.find('surveyLocationQualifier').text,
                    ...
                    for parent_rate_type in ('occupation', 'rate'):
                        if rate_type != parent_rate_type:
                            try:
                                parent = rate.iterancestors(
                                    parent_rate_type).__next__()
                                result['occupation'][parent_rate_type] = {
                                    'title': text(parent.find(parent_rate_type
                                                              + 'Title')),
                                    'qualifier': text(parent.find(
                                        parent_rate_type + 'Qualifier')),
                                    'groupQualifier': text(parent.find(
                                        parent_rate_type + 'GroupQualifier')),
                                }
